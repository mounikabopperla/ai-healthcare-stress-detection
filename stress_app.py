...
with col1:
    ...
    occupation = st.selectbox("Occupation", ["Student", "Software Engineer", "Healthcare", "Teacher", "Other"])
    marital_status = st.selectbox("Marital Status", ["Single", "Married", "Other"])
    sleep_duration = st.number_input("Sleep Duration", min_value=0.0, max_value=15.0, value=6.0)
    sleep_quality = st.number_input("Sleep Quality", min_value=1.0, max_value=5.0, value=3.0)

with col2:
    wake_up_time = st.text_input("Wake Up Time", "7:00 AM")
    bed_time = st.text_input("Bed Time", "10:00 PM")
    physical_activity = st.number_input("Physical Activity", min_value=0.0, max_value=30.0, value=3.0)
    screen_time = st.number_input("Screen Time", min_value=0.0, max_value=24.0, value=6.0)
...
with col3:
    work_hours = st.number_input("Work Hours", min_value=0.0, max_value=24.0, value=8.0)
    travel_time = st.number_input("Travel Time", min_value=0.0, max_value=10.0, value=1.0)
    social_interactions = st.number_input("Social Interactions", min_value=0, max_value=20, value=5)
    meditation = st.selectbox("Meditation Practice", ["Yes", "No"])
    exercise_type = st.selectbox("Exercise Type", ["Cardio", "Yoga", "Strength", "None", "Other"])
    ...
    blood_sugar = st.number_input("Blood Sugar Level", min_value=60, max_value=250, value=90)
